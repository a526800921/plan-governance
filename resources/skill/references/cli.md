...
仅更新计划格式不等于更新项目入口；更新 AGENTS/CLAUDE 入口也不等于当前阶段已采用风险分流。已有项目在任务交接、没有进行中的实施或复核时更新受管入口；阶段策略单独按 verification 兼容规则处理，不批量回写历史。安装副本与仓库开发规则有差异时说明本次选用来源，不能因版本号相同就混读正文。
