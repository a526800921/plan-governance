...
默认不启用自主连续执行。只有当计划明确选择“推进到完成计划”并且每一步都需要真实执行时，才复制下面的示例到当前阶段，去掉代码围栏并替换所有占位内容。未启用时，旧计划和普通计划继续使用原有流程，不需要补填步骤表。

只读查询入口为 `plan-governance-cli plan steps validate <plan> --json` 和 `plan-governance-cli plan next <plan> --json`；查询不会执行动作、写回状态或替代独立验收。
