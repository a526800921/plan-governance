...
- `scripts/graph_governance.mjs` 增加只读 `plan impact --input`，`bin/plan-governance-cli.mjs` 增加 `plan` 子命令转发。
- `plan impact` 优先读取 `docs/graph/functional/index.yaml`；当前 ModelPad 尚未切换到新入口时，只读回退 `docs/graph/functional.yaml`，并在影响遍历阶段排除 `api.*`、`code.*`、`test.*`、`document.*` 等旧混合节点，不迁移或写回旧文件。
- API 契约变更沿架构层 `realized_by` 和 `exposes` 展开；数据/安全变更沿 `crosses` 展开；代码定位只调用候选解析，不自动刷新 GitNexus。
...
- ModelPad 首批只写入 3 个架构边界的 `code_mappings[].code_anchor`，不写 GitNexus UID；映射完成后通过架构层校验和代码级影响回放验收。

#### 阶段 2 完成条件

- ModelPad `architecture/index.yaml` 加载独立 `code-mappings.yaml`，包含本地 HTTP API、配置持久化和模型进程管理 3 个稳定代码锚点。
- 架构层校验通过，代码锚点的 `file + symbol + kind` 唯一且文件存在；首批映射不包含 GitNexus UID。
- 候选报告 CLI 的唯一、多候选 `ask_user`、无候选和 UID 失配串联回放均可复现；代码影响 CLI 的正常查询和 GitNexus 错误查询均返回正确语义。
- 功能图谱和 Swift 代码不变，GitNexus 索引不刷新；ModelPad `detect-changes --scope unstaged` 不报告代码符号变更。
- 根仓库 32/32 测试、治理检查、严格准入检查和反向引用检查通过。
- 阶段 2 独立完成复核明确通过后，才将计划状态和 `PLAN_MAP.md` 更新为 `已完成`，并把当前阶段推进到阶段 3 规划。
...
| 阶段 | 阶段 3 |
| 结论 | 通过；达到待实施标准 |
| 证据 | 五个 Step 0 样本逐项断言 `queried_layers`、升级原因、`unqueried_layers`、功能节点集合和行动分级；旧混合图谱中的 `api.*`、`code.*`、`test.*`、`document.*` 已排除；4 类非法输入拒绝通过；新增配置刷新 API 契约样本并补齐本地 HTTP API 架构映射；根仓库 37/37 测试、治理检查和严格准入检查通过；全程无写回且未触发 `gitnexus analyze` |
| 未满足条件 | 无；生产命令仍需在实施阶段补齐命令级悬空节点、架构映射缺失、多候选和 GitNexus 错误测试 |
| 复核者 | James 独立复核代理（基于当前仓库内容和可复现命令） |
...
| 2026-07-25 | 阶段准入复核 | 阶段 3 | 未通过 | 初次复核指出旧 `graph impact` 基线未逐项验证新分层契约；补充参考决策回放、旧混合节点隔离和非法输入测试后重新复核 | James 独立复核代理 |
| 2026-07-25 | 阶段准入复核 | 阶段 3 | 通过；达到待实施标准 | 四个样本逐项断言分层决策、升级原因、行动分级和测试映射；旧混合节点隔离；4 类非法输入测试；33/33 测试、治理和严格准入检查通过；允许进入生产 `plan impact` 实现 | James 独立复核代理 |
| 2026-07-25 | 阶段完成后续复核 | 阶段 3 | 通过；阶段 3 完成证据保持有效 | 五个样本逐项断言分层决策、升级原因、行动分级和测试映射；配置刷新 API 契约已补齐本地 HTTP API 架构映射；37/37 测试、治理和严格准入检查通过；未触发 `gitnexus analyze` | James 独立复核代理 |
