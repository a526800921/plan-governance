...
| [architecture-graph-governance](plans/architecture-graph-governance.md) | 设计中 | 阶段 3 | 2026-07-25 | functional-graph-governance | [当前阶段](plans/architecture-graph-governance.md#当前阶段) |
...
13. `architecture-graph-governance`（设计中：阶段 2 已完成，阶段 3 开始设计计划前置影响分析）
...
|---|---|---|---|---|
| 阶段 3 Step 0 已完成真实仓库回放，但尚未完成生产命令契约测试和独立准入复核 | 先基于四个真实样本冻结 `plan impact` 输入/输出和失败语义，再进行独立准入复核；复核前不实现生产命令 | architecture-graph-governance 阶段 3 | 是 | 进行中 |
