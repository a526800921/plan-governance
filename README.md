...
先利用上下文和工具查证需求，在已有授权内推进。只有必须由用户决定、且会实质影响结果或授权边界的问题才询问，已有决定不重复确认。

`grill-me` 是深入探索时的可选方法，缺失不阻塞任务。需要治理时把重要决定和未决问题记录到当前计划；普通可逆细节自主处理，等待必要答案时继续无依赖的授权工作。
...
```bash
plan-governance-cli guide                 # 入口
plan-governance-cli guide planning        # 计划/契约/探索
plan-governance-cli guide verification    # 阶段门/复核/验证/用户验收
plan-governance-cli guide cli             # CLI 操作
```

读取当前包内正文，与 cwd 无关，不需要 Python，不下载或修改项目。未知主题、多余参数、缺失或空资源非零退出。项目受管入口依赖已安装 skill 或具备上述能力的 CLI；两者均不可用时明确说明前提，不自动安装。
...
- 验收治理文档时必须做反向引用检查。
- 按适用风险复核规范核对当前内容、可复验结果和反向引用；状态文字不是验收证据。
- 启用治理后，旧草案停止承载新规范；后续需求默认进入治理文档。
- 如果新信息改变计划顺序、公共契约、兼容承诺或完成条件，先更新治理文档，再继续实施。

现行规则从 [skill 入口](resources/skill/SKILL.md)按需读取；[原始设计](plan-governance-design.md)仅作历史背景。
